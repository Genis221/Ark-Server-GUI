...
function Ensure-WindowsStartup {
  try {
    ...
    # Remove the old broken .lnk approach if it exists.
    ...
    $vbs = @"
' Auto-generated by Ark Server Manager - launches at Windows logon.
Option Explicit
Dim sh
Set sh = CreateObject("WScript.Shell")
sh.Run "wscript.exe //B //Nologo ""$escaped""", 0, False
"@
    Set-Content -LiteralPath $entryPath -Value $vbs -Encoding ASCII -Force
