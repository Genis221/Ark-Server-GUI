...
function windowsStartupEntryPath() {
  return path.join(windowsStartupDir(), "ArkServerManager.vbs");
}

async function clearWindowsStartupEntries() {
  const entryPath = windowsStartupEntryPath();
  const legacyLnk = path.join(windowsStartupDir(), "Ark Server Manager.lnk");
  await rm(entryPath, { force: true });
  await rm(legacyLnk, { force: true });
  try {
    await execFileAsync(
      "powershell.exe",
      [
        "-NoLogo", "-NoProfile", "-NonInteractive", "-Command",
        "Remove-ItemProperty -Path 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Run' -Name 'ArkServerManager' -ErrorAction SilentlyContinue"
      ],
...
async function setWindowsStartupEnabled(enabled) {
  ...
  if (!enabled) {
    await clearWindowsStartupEntries();
    return false;
  }
  const launcher = path.join(ROOT, "StartArkManagerAtLogon.vbs");
  if (!(await pathExists(launcher))) {
    throw Object.assign(new Error("StartArkManagerAtLogon.vbs was not found next to server.mjs"), { status: 500 });
  }
  const startupDir = windowsStartupDir();
  await mkdir(startupDir, { recursive: true });
  await rm(path.join(startupDir, "Ark Server Manager.lnk"), { force: true });
  const entryPath = windowsStartupEntryPath();
  const escaped = launcher.replace(/"/g, '""');
  const vbs = `' Auto-generated by Ark Server Manager - launches at Windows logon.
Option Explicit
Dim sh
...
async function syncManagerWindowsStartup() {
  ...
  try {
    return await setWindowsStartupEnabled(want);
  } catch (err) {
...
async function handleApi(req, res, url) {
  ...
  if (method === "POST" && pathname === "/api/manager/startup") {
    ...
    state.managerStartWithWindows = enabled;
    scheduleSave();
    try {
      await setWindowsStartupEnabled(enabled);
    } catch (err) {
      return sendJson(res, err.status || 500, { error: err.message || "Could not update Windows startup" });
    }
    addActivity(
